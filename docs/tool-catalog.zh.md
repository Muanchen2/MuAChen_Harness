...
| `@deepseek-ai/dsh-skill-admin` | `skill-admin` | `ctx.tools`、`ctx.skills`、`ctx.systemPrompt`、`ctx.subprocess` | `tool/call`、`tool/result`、`git commits in the skill store` | - | skill-admin 工具渲染 git 底座的技能存储操作；schema 采集挂载本地子进程运行时，使服务构造函数无需触碰真实 store 即可解析。 |
| `@deepseek-ai/dsh-tool-memory` | `memory` | `ctx.tools`、`ctx.memories`、`ctx.systemPrompt` | `tool/call`、`tool/result`、`git commits in the memory store` | - | memory 工具渲染 Rin 的 git 支撑存储操作；schema 采集挂载本地子进程运行时，使服务构造器无需触碰真实 store 即可解析。 |
...
管理 git 底座的技能存储（写/维护面，区别于只读的 skill 工具）：create、update、archive、remove、list、history、revert、commit 和 promote 技能。
...
来源：[`packages/skill/skill-admin/src/index.ts`](../packages/skill/skill-admin/src/index.ts)

skill-admin 工具渲染 git 底座的技能存储操作；schema 采集挂载本地子进程运行时，使服务构造函数无需触碰真实 store 即可解析。
...
持久化或回顾跨会话的项目经验（memory），区别于 skills（方法）。
...
```json
{
  ...
  "properties": {
    ...
    "id": {
      "type": "string",
      "description": "Memory node id for read/timeline, or the id to save under for remember."
    },
    "query": {
      "type": "string",
      "description": "Full-text search query for memory search (case-insensitive literal match over node bodies)."
    },
    "revision": {
      "type": "string",
      "description": "Git revision to restore for revert (take it from timeline output)."
    },
    "title": {
      "type": "string",
      "description": "Title for remember; the new memory heading."
    },
    "content": {
      "type": "string",
      "description": "Body for remember; the experience to persist."
    },
    "message": {
      "type": "string",
      "description": "Optional one-line commit note for this change."
    },
    "strategy": {
      "type": "string",
      "description": "Conflict resolution for merge: ours (keep target) or theirs (keep merged)."
    },
    "prefix": {
      "type": "string",
      "description": "Optional id prefix filter for list (e.g. archive/ to browse the archive, handoff/ for handoff memos)."
    }
  },
  "required": [
    "action",
...
memory 工具渲染 Rin 的 git 支撑存储操作；schema 采集挂载本地子进程运行时，使服务构造器无需触碰真实 store 即可解析。
