...
const MEMORY_GUIDANCE = '使用 memory 工具持久化与回顾跨会话的项目经验，区别于 skills（方法）：'
  ...
  + '因此现在记录的经验在未来会话中自动可见。**central 写入需主人审核**：AI 不得直接写 central。'
  + '判断某条经验有跨项目通用价值时，先写入 workspace 的 central-candidate/<主题>（内容 + 跨项目理由），'
  + '并在回复中明确提示主人审核；主人许可后才执行 memory remember（scope=central）正式写入，'
  + '随后归档（archive）候选；主人否决则保留或移除候选。开始长任务前，先用 memory list/read 回顾相关记忆，'
